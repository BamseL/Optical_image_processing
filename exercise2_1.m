...
for i=1:height
    for j=1:width
        s3=sum(sum(1./f_d_r3(i:i+2,j:j+2)));
        f_L3(i+(N3-1)/2,j+(N3-1)/2)=s3.*(N3*N3);
    end
end
...
for i=1:height
    for j=1:width
        s5=sum(sum(1./f_d_r5(i:i+4,j:j+4)));
        f_L5(i+(N5-1)/2,j+(N5-1)/2)=s5.*(N5*N5);
    end
end
...
for i=1:height
    for j=1:width
        s9=sum(sum(1./f_d_r9(i:i+8,j:j+8)));
        f_L9(i+(N9-1)/2,j+(N9-1)/2)=s9.*(N9*N9);
    end
end
